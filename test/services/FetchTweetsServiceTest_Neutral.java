...
/**
 * JUnit test cases for FetchTweetsServiceTest_Neutral .
 *
 * @author Samiparna
 * @version 1.0.0
 */

public class FetchTweetsServiceTest_Neutral {
    ...
    public void setupData() {
        ...
        Twitterserver = Server.forRouter((components) -> RoutingDsl.fromComponents(components)
                ...
                .GET("/1.1/search/tweets.json").routeTo(() -> ok().sendResource("result_search_neutral.json"))
                .build());
    ...
    /**
     * Verifies that the testFetchTweetsfromAPI returns correct response to the dummy input for neutral emoticon
     */
    @Test
    public void testFetchTweetsfromAPI() throws InterruptedException, ExecutionException {
        ...
        Map<String, List<Tweet_Object>> TestData = FetchTweetService.FetchTweetsfromAPI(Arrays.asList("Montreal", "Cafe"),10).toCompletableFuture().get();
        System.out.println(TestData);
        assertTrue(TestData.get("Montreal --   :-| 100%").size() == 3);
        assertTrue(TestData.get("Cafe --   :-| 100%").size() == 3);
    }
}